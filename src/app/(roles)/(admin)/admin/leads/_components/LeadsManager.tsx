      ...
      <div className="bg-white/80 dark:bg-[#111111]/80 backdrop-blur-xl rounded-[2rem] border border-gray-200/80 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] overflow-hidden flex flex-col">
        ...
        <div className="sm:hidden space-y-4 p-4">
          ...
          {isLoading ? (
            ...
            <div className="p-8 text-center text-gray-500 font-bold bg-gray-50 dark:bg-white/5 rounded-2xl border border-gray-100 dark:border-white/10">No leads found.</div>
          ) : (
            sortedLeads.map((lead) => (
              <div key={lead.id} className="bg-white dark:bg-[#1a1a1a] p-4 rounded-xl border border-gray-100 dark:border-white/5 shadow-sm space-y-3">
                <div className="flex justify-between items-start gap-2">
                  <div>
                    <p className="font-bold text-gray-900 dark:text-white">{lead.fullName || lead.businessName}</p>
                  ...
                  <span className={`px-2 py-1 text-[10px] font-bold rounded-md uppercase border ${
                    lead.status === 'NEW' ? 'bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-500/10 dark:text-blue-400' :
                    lead.status === 'CONTACTED' ? 'bg-indigo-50 text-indigo-600 border-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-400' :
                    lead.status === 'INTERESTED' ? 'bg-amber-50 text-amber-600 border-amber-200 dark:bg-amber-500/10 dark:text-amber-400' :
                    lead.status === 'CONVERTED' ? 'bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-500/10 dark:text-emerald-400' :
                    lead.status === 'LOST' ? 'bg-red-50 text-red-600 border-red-200 dark:bg-red-500/10 dark:text-red-400' :
                ...
                <div className="bg-gray-50 dark:bg-white/5 p-3 rounded-lg mt-2 flex flex-col gap-1.5">
                  <div className="flex items-center gap-2 text-xs">
                    ...
                    <span className="font-medium text-gray-700 dark:text-gray-300">{lead.serviceInterest || '—'}</span>
                  </div>
                  <div className="flex items-center gap-2 text-xs">
